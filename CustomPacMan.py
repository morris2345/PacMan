...
class CustomPacManEnv(gym.Env):
        ...
        for i in range(self.ghost_count):
            if ((self.ghosts[i] == new_pos) or (new_pos == old_ghost_positions[i] and current_PacMan_pos == self.ghosts[i])): #if ghost on location of pacman or pacman and ghost moved through eachother
                if(self.pill_active == True):#if pill active eat ghost
                    ...
                    self.ghosts_eaten += 1
                elif(self.food_count > 0 and self.pill_active == False): #if pill not active and pacman didn't just eat last food pellet(winning the game) pacman gets eaten
                    self.lives -= 1
    ...
    def getState(self):
        ...
        #in what direction the ghosts are from PacMan
        ghost_directions = []
        for i in range(self.ghost_count):
        ...
        #if there is a ghost in squares next to PacMan or not
        ghost_in_directions = []# is there a ghost in a direction next to pacman
        for (direction_x, direction_y) in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            next_x, next_y = x + direction_x, y + direction_y # x,y next to PacMan
            if ((next_x, next_y) in self.ghosts): #if x,y coordinates are in ghosts position list
                ghost_in_directions.append(1) #append 1 if ghost is next to PacMan
            else:
                ghost_in_directions.append(0)

        #postions of all food pellets
        ...
        return (state, self.rm_state) # Return state + Reward Machine State. (RM State will always be 0 for QLearning without Reward Machine)
    ...
    def updateRMState(self):
        if(self.algo == "QRM"):#only update if using Q-Learning with Reward Machine
            if self.rm_state == 0 and self.pill_active: #If in non powered state and pill active
                ...
                if(self.version == "Normal"): #change rewards
